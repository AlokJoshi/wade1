...
Animation.prototype._drawSingle = function(context, position, size, uvMinX, uvMinY, uvMaxX, uvMaxY)
{
    wade.numDrawCalls++;
    context.drawImage(this._image, this._frameCorner.x, this._frameCorner.y, this._frameSize.x, this._frameSize.y, position.x - size.x / 2 + this._offset.x, position.y - size.y / 2 + this._offset.y, size.x, size.y);
};
...
Animation.prototype._drawBlended = function(context, position, size)
{
    ...
    var otherCorner = {x: (otherFrame % this._numCells.x) * this._frameSize.x + this._image.width * this._imageArea.minX, y: Math.floor(otherFrame / this._numCells.x) * this._frameSize.y + this._image.height * this._imageArea.minY};
    wade.numDrawCalls += 2;
    ...
    context.drawImage(this._image, this._frameCorner.x, this._frameCorner.y, this._frameSize.x, this._frameSize.y, destX, destY, size.x, size.y);
    context.globalCompositeOperation = 'lighter';
    context.globalAlpha = alpha * this._frameFraction;
    context.drawImage(this._image, otherCorner.x, otherCorner.y, this._frameSize.x, this._frameSize.y, destX, destY, size.x, size.y);
...
Animation.prototype.draw_gl = function(context, positionAndSize, rotationAlpha)
{
    ...
    wade.numDrawCalls++;
    context.uniform4fv(context.currentShader.uniforms['uPositionAndSize'], positionAndSize);
    context.uniform4fv(context.currentShader.uniforms['uAnimFrameInfo'], this._f32AnimFrameInfo);
    context.uniform2fv(context.currentShader.uniforms['uRotationAlpha'], rotationAlpha);
