...
function Wade_drawFunctions()
{
    ...
    this.gradientFill_ = function(gradientDirection, colors)
    {
        return function(context)
        {
            ...
            var e = this._layer? wade.screenUnitToWorld(this._layer.id) * 0.5 : 0.5;
            ...
            {
                wade.warn('Warning: attempting to draw a sprite with an invalid linear gradient');
            }
        }
    };
    ...
    this.solidFill_ = function(color)
    {
        return function(context)
        {
            ...
            context.fillRect(this._position.x - this._size.x / 2, this._position.y - this._size.y / 2, this._size.x, this._size.y);
    ...
    this.grid_ = function(numCellsX, numCellsY, color, lineWidth)
    {
        return function(context)
        {
            ...
            var e = (this._layer? wade.screenUnitToWorld(this._layer.id) * 0.5 : 0.5) * lineWidth;
     ...
     */
    this.additive_ = function(originalDrawCall)
    {
