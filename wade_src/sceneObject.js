...
SceneObject.prototype.serialize = function(stringify, propertiesToExclude, serializeFunctions)
{
    ...
    for (i=0; i<this._spriteOffsets.length; i++)
    {
        ...
        result.spriteOffsets.push({x: ox, y: oy, angle: this._spriteOffsets[i].angle});
    }
    for (i=0; i<this._sprites.length; i++)
    {
...
SceneObject.prototype.importFunctions = function(functionObject)
{
    ...
    if (debugMode)
    {
        functionsScript += '\n//# sourceURL=_.' + this._name.replace(/ /g, '_');
        try
        {
 ...
 * @param {boolean} [setInitialValues] Whether the object and its sprite should be scanned to set the initial values of all properties that are used on Node 1 (the second node) of the path. Note that this effectively creates a copy of the path, it doesn't modify the original one.
