...
function Wade()
{
     ...
     * @param {Object} [options = {}] An object that contains a set of fields that specify some configuration options. All the fields are optional. Supported values are:<br/><ul>
        ...
        <li> <b>forceWebGl</b>: <i>boolean</i> - Whether to use WebGl even when the browser detects potential performance problems. Defaults to false.</li></ul>
     ...
     * @param {object} data A scene description object, such as one created with wade.exportScene(). The format is the following (all fields are optional):<pre>
       {
            <b>json</b>: An array of file names, describing which json files should be loaded. This can also be an array of objects in the format {resource: string, target: string} where <i>resource</i> is the file to load, and the result is stored in wade.app[<i>target</i>].
            <b>audio</b>: An array of audio file names
            <b>scripts</b>: An array of script (.js) file names. Note that these scripts will be loaded and executed after the rest of the scene has been loaded, but before any scene objects are created and added to the scene
            <b>images</b>: An array of image file names - you don't need to include files that are referenced by the scene objects and sprites in the scene (those will be loaded automatically).
            <b>minScreenSize</b>: An object with x and y components describing the minimum screen size. Refer to the documentation of wade.setMinScreenSize() for more details
            <b>maxScreenSize</b>: An object with x and y components describing the maximum screen size. Refer to the documentation of wade.setMaxScreenSize() for more details
            <b>windowMode</b>: A string describing the window mode. Refer to the documentation of wade.setWindowMode() for more details
            <b>orientation</b>: A string describing the orientation. Valid values are 'portrait' and 'landscape', all other values are ignored. See wade.forceOrientation() for more details
            <b>sceneObjects</b>: An array containing all the SceneObjects to instantiate. See the SceneObject documentation for more details about the format to use
            <b>modules</b>: An object where the name of each property is the name of an external WADE module whose importScene is called to load the scene data described by the value of that property
            <b>webAudioOnly</b>: A boolean describing whether audio should only be loaded through WebAudio, and only where WebAudio is supported
            <b>loadGlTextures</b>: A boolean describing whether WebGl textures should be created at loading time (only applies to sprites that are on WebGl layers)
       }</pre>
                ...
                if (data.layers)
                {
                    for (i=0; i<data.layers.length; i++)
                    {
                        if (data.layers[i])
                        {
                            ...
                            wade.setLayerRenderMode(i, (data.layers[i].renderMode == '2d')? '2d' : 'webgl');
    ...
    {
        this.getLayer(layerId, renderMode).setRenderMode(renderMode);
    };
